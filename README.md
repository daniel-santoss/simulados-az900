...
```bash
git clone https://github.com/<seu-usuario>/az900-simulados.git
cd az900-simulados
# abra index.html com um duplo clique, ou:
python3 -m http.server 8000   # depois acesse http://localhost:8000
```

Abrir o arquivo direto funciona porque o projeto usa apenas HTML/CSS/JS puro, sem build. Usar um servidor local (o comando acima) é opcional, mas garante que o `localStorage` do histórico funcione em qualquer navegador.

**No GitHub Pages:** em *Settings → Pages*, publique a branch `main` (pasta `/root`). O app roda direto, sem configuração.
...
Sem licença formal — sinta-se livre para estudar, sugerir melhorias e adaptar. Se for redistribuir, dê os devidos créditos.